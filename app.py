...
# --- 6. DASHBOARD TAB (Updated with Revenue Variance) ---
with tab_dash:
    jobs_df = get_db_jobs()
    if not jobs_df.empty:
        ...
        # Calculations for Revenue Variance
        ...
        m2.metric("Annual Revenue Target", f"{CURRENCY}{ANNUAL_REVENUE_TARGET:,.2f}")
        m3.metric("Variance to Target", f"{CURRENCY}{variance:,.2f}", delta=f"{variance:,.2f}")
...
# --- 7. REMAINING TABS (Maintained exactly as requested) ---
with tab_plan:
